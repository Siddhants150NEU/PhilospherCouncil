...
import { camusStageManifest, type CamusStageState } from "./camusStageManifest";

type CamusFallbackBodyProps = {
  gesture?: string;
  size?: number;
};

interface CamusMiniAvatarProps {
  active?: boolean;
  label?: string;
  sublabel?: string;
  compact?: boolean;
  FallbackBody: ComponentType<CamusFallbackBodyProps>;
}

interface CamusProofStageProps {
  currentState: CamusStageState;
  currentLine: string;
...
function stageGestureFor(state: CamusStageState): "idle" | "speak" | "point" {
  if (state === "speaking") return "speak";
  if (state === "reflecting") return "point";
  return "idle";
}

export function CamusMiniAvatar({
  active = false,
  label = "Camus",
  sublabel = "2.5D stage slot",
  compact = false,
  FallbackBody,
}: CamusMiniAvatarProps) {
  const fallbackGesture = active ? "speak" : "idle";
  const avatarSrc = compact
    ? camusStageManifest.proofAssets.avatar.icon128
    : active
      ? camusStageManifest.proofAssets.avatar.bustActive
      : camusStageManifest.proofAssets.avatar.bustNeutral;
  return (
    <div
      ...
      <div className={`relative flex items-end justify-center ${compact ? "h-14" : "h-24"}`}>
        {camusStageManifest.proofAssets.ready ? (
...
export function CamusProofStage({
  currentState,
  currentLine,
  selectedCount,
  FallbackBody,
}: CamusProofStageProps) {
  const stateSpec = camusStageManifest.actor.states[currentState];
  const fallbackGesture = stageGestureFor(currentState);
  const proofAssets = camusStageManifest.proofAssets;
  ...
  const actorSizingStyle =
    currentState === "reflecting"
      ? { height: "min(540px, 44vw, 56vh)" }
      : currentState === "speaking"
        ? { height: "min(640px, 48vw, 64vh)" }
        : { height: "min(610px, 46vw, 61vh)" };
  const actorPlacementClass =
    currentState === "reflecting"
      ? "translate-y-12 md:translate-y-16"
      : currentState === "speaking"
        ? "translate-y-18 md:translate-y-24"
          ...
          <div className="pointer-events-none absolute inset-x-0 top-0 z-[7] flex justify-center px-4 pb-6 pt-[4.9rem] md:px-8 md:pt-[5.7rem]">
            <div className="camus-stage-header-panel w-full max-w-[560px] rounded-[22px] px-4 py-3 md:px-6">
              <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3">
                <div className="font-display text-[1.65rem] leading-none text-[#e59a6b] md:text-[2.2rem]">Camus</div>
            ...
            <div className={`camus-actor-wrap relative ${actorPlacementClass}`}>
              {proofAssets.ready ? (
                <div
                  ...
                  <img
                    src={actorSrc}
                    alt={`Albert Camus ${stateSpec.title.toLowerCase()} concept art`}
                ...
                <div className="camus-fallback-actor relative">
                  ...
                  <div className={`relative ${stateAnimationClass}`}>
                    <FallbackBody size={280} gesture={fallbackGesture} />
                  </div>
                </div>
              )}
            </div>
